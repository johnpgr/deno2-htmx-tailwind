function startAutoRefresh(refreshNow = false, restartDelay = 0) {
    ...
    evtSource.addEventListener("error", ()=>{
        evtSource.close();
        if (restartDelay < 30) {
            setTimeout(()=>startAutoRefresh(true, restartDelay + 1), restartDelay * 100);
        } else {
            console.log("%cAUTO REFRESH DISABLED%c - you'll need manually refresh, ok", "color: red", "color: unset");
        }
    });
}
